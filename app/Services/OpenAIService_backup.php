...
class OpenAIService
{
    ...
    {
        // TODO: Implement new LangChain + Chroma RAG system
        return [
            'success' => false,
            'message' => 'AI chat system is being rebuilt with LangChain and Chroma.',
            'error' => 'System under reconstruction'
        ];
    }
    ...
    private function buildConciseAnswerFromKb(array $kb, string $question = ''): string
    {
        ...
        if ($html === '' && !empty($kb['excerpt'])) $html = (string)$kb['excerpt'];
        if ($html === '' && !empty($kb['question'])) $html = (string)$kb['question'];
        if ($html === '' && !empty($kb['search_content'])) $html = (string)$kb['search_content'];
        $text = $this->normalizeHtmlToText($html);
        if ($text === '') {
            $alt = '';
            $fields = [];
            if (!empty($kb['answer'] ?? '')) $fields[] = (string)$kb['answer'];
            if (!empty($kb['excerpt'] ?? '')) $fields[] = (string)$kb['excerpt'];
            if (!empty($kb['question'] ?? '')) $fields[] = (string)$kb['question'];
            if (!empty($kb['search_content'] ?? '')) $fields[] = (string)$kb['search_content'];
            if (!empty($kb['title'] ?? '')) $fields[] = (string)$kb['title'];
            if (!empty($fields)) $alt = implode("\n", $fields);
            $text = $this->normalizeHtmlToText($alt);
            if ($text === '') return '';
        }
        $keywords = $this->extractKeywords($question);

        // Special handling: extract schedule day-location and hours if present
        $lower = mb_strtolower($text);
        $dayRegex = '(Senin|Selasa|Rabu|Kamis|Jum[\'\’`´]?at|Jumat|Sabtu|Minggu|Ahad)';
        $hasDay = (bool) preg_match('/\b(' . $dayRegex . ')\b/iu', $lower);
        if ($hasDay) {
            // Normalize: insert a space before day names if glued to previous word (e.g., "SukodadiSelasa")
            $t = preg_replace('/(?<=\p{L})(?=' . $dayRegex . ')/iu', ' ', $text);

            // Helper to sanitize extracted location text
            $cleanLoc = function (string $loc) use ($dayRegex): string {
                $loc = trim($loc);
                // Remove any day tokens accidentally included in the location
                $loc = preg_replace('/\b' . $dayRegex . '\b/iu', '', $loc);
                // Remove leading separators or filler words
                $loc = preg_replace('/^\s*(\:|\-|—|–|di)\s*/iu', '', $loc);
                // Collapse spaces
                $loc = trim(preg_replace('/\s+/', ' ', $loc));
                return $loc;
            };

            // Extract day-location pairs stopping at the next day token
            $pairs = [];
            // Pattern 1: "Hari di|: Lokasi" until next day token
            $pattern = '/\b(' . $dayRegex . ')\b\s*(?:di|:|\-|—|–)\s*([^\n\r]*?)(?=(?:\b' . $dayRegex . '\b)|$)/iu';
            if (preg_match_all($pattern, $t, $m, PREG_SET_ORDER)) {
                foreach ($m as $row) {
                    $day = trim($row[1]);
                    $loc = $cleanLoc((string)$row[2]);
                    // Clean noisy tails commonly found after last item
                    $loc = preg_replace('/\b(Samsat\s+Keliling.*)$/iu', '', $loc);
                    $loc = preg_replace('/\b(Jadwal\s+Samsat.*)$/iu', '', $loc);
                    $loc = preg_replace('/\b(lokasi)\b.*$/iu', '', $loc);
                    $loc = preg_replace('/\s+jam\s+[0-2]?\d.*/iu', '', $loc);
                    $loc = trim(preg_replace('/\s+/', ' ', $loc));
                    if ($loc === '') continue;
                    // Normalize day spelling
                    if (preg_match('/^Jum/iu', $day)) $day = 'Jumat';
                    $pairs[$day] = $loc;
                }
            }

            // If no pairs found from plain text, try building from HTML list bullets
            if (empty($pairs)) {
                $bulletItems = $this->extractBullets($html);
                if (!empty($bulletItems)) {
                    foreach ($bulletItems as $bi) {
                        if (preg_match('/^\s*(' . $dayRegex . ')\b\s*(.*)$/iu', $bi, $bm)) {
                            $day = trim($bm[1]);
                            $rest = trim($bm[2]);
                            if (preg_match('/^(:|\-|—|–)\s*(.+)$/u', $rest, $mmSep)) {
                                $rest = (string)$mmSep[2];
                            }
                            if (preg_match('/\bdi\s+(.+)/iu', $rest, $mm2)) {
                                $loc = trim($mm2[1]);
                            } else {
                                $loc = trim($rest);
                            }
                            $loc = $cleanLoc((string)$loc);
                            if ($loc !== '') {
                                if (preg_match('/^Jum/iu', $day)) $day = 'Jumat';
                                $pairs[$day] = $loc;
                            }
            ...
            // If still no pairs, fallback to crude line capture
            // If no pairs found, fallback to crude line capture
            if (empty($pairs) && preg_match_all('/\b' . $dayRegex . '\b[^\n\r]*/iu', $t, $mm)) {
                foreach ($mm[0] as $ln) {
                    $line = trim(preg_replace('/\s+/', ' ', $ln));
                    // Try split Day + rest
                    if (preg_match('/^\b(' . $dayRegex . ')\b\s*(.*)$/iu', $line, $cap)) {
                        $day = trim($cap[1]);
                        $rest = trim($cap[2]);
                        // Accept common separators if present
                        if (preg_match('/^(:|\-|—|–)\s*(.+)$/u', $rest, $mmSep)) {
                            $rest = (string)$mmSep[2];
                        }
                        if (preg_match('/\bdi\s+(.+)/iu', $rest, $mm2)) {
            ...
            // Extract hours if present; accept variants like "Jam 18.30 sampai 20.00 WIB" or "18.30–20.00"
            $hours = '';
            if (preg_match('/(?:jam\s*)?([0-2]?\d(?:[.:]\d{2})?)\s*(?:s\.?:?d\.?|s\/?d|sampai|hingga|\-|—|–)\s*([0-2]?\d(?:[.:]\d{2})?)\s*(?:wib|wita|wit)?/iu', $t, $hm)) {
                $h1 = $hm[1];
                $h2 = $hm[2];
                $hours = htmlspecialchars("Jam pelayanan: {$h1}–{$h2} WIB");
            }

            if (!empty($pairs)) {
                $order = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu', 'Ahad'];
                $items = [];
                foreach ($order as $d) {
                    if (isset($pairs[$d])) {
                        $loc = trim((string)$pairs[$d]);
                        if ($loc === '' || strcasecmp($loc, $d) === 0) continue; // skip bad "Day: Day" entries
                        // Enforce no leading 'di ' in final rendering
                        $loc = preg_replace('/^di\s+/iu', '', $loc);
                        $items[] = htmlspecialchars($d . ': ' . $loc);
                    }
                }
                // Append any extra days that slipped through
                foreach ($pairs as $d => $loc) {
                    if (!in_array($d, $order, true)) {
                        $loc = trim((string)$loc);
                        $loc = preg_replace('/^di\s+/iu', '', $loc);
                        if ($loc === '' || strcasecmp($loc, (string)$d) === 0) continue;
                        $items[] = htmlspecialchars($d . ': ' . $loc);
                    }
                }
                if (empty($items)) {
                    // If nothing valid, fall back to raw KB text to avoid empty fast-path
                    return htmlspecialchars(mb_substr($text, 0, 500));
                }
                $htmlOut = '<ul><li>' . implode('</li><li>', $items) . '</li></ul>';
                if ($hours !== '') $htmlOut .= '<div>' . $hours . '</div>';
                return $htmlOut;
            }
        }
        ...
        $out = trim(implode(' ', array_map('htmlspecialchars', $picked)));
        if ($out !== '') return $out;
        // Fallback: line-based slicing for schedule-like text
        $lines = preg_split('/\n+/u', $text) ?: [];
        $lines = array_values(array_filter(array_map('trim', $lines), fn($l) => $l !== ''));
        if (!empty($lines)) {
            $picked = $this->pickRelevantLines($lines, $keywords, 6);
            if (empty($picked)) $picked = array_slice($lines, 0, 6);
            return '<ul><li>' . implode('</li><li>', array_map('htmlspecialchars', $picked)) . '</li></ul>';
        }
        return '';
    }
    ...
    private function keywordOverlapCountForKb(array $kb, string $userText): int
    {
        ...
        $tagsStr = '';
        if (isset($kb['tags'])) {
            if (is_array($kb['tags'])) $tagsStr = implode(' ', array_map('strval', $kb['tags']));
            else $tagsStr = (string) $kb['tags'];
        }
        $hay = strtolower(
            ($kb['title'] ?? '') . ' ' .
                ($kb['question'] ?? '') . ' ' .
    ...
    /**
     * Detect high-level user intents from the query text.
     */
    private function detectIntents(string $text): array
    {
        ...
        $map = [
            // Pembayaran & cek
            'check_tax' => ['cek', 'check', 'cek pajak', 'cek biaya', 'cek tarif', 'tarif', 'biaya', 'lihat pajak', 'esamsat', 'pira pajek', 'pinten pajek', 'ndelok pajek', 'ncek pajek'],
            'online_payment' => ['bayar online', 'pembayaran online', 'e-samsat', 'mbanking', 'm-banking', 'atm', 'virtual account', 'va', 'bank jatim', 'bri', 'bni', 'mandiri', 'dana', 'ovo', 'shopee', 'mbayar online'],
            'offline_location_hours' => ['lokasi', 'alamat', 'jam operasional', 'jam buka', 'jam bukak', 'tutup', 'buka sabtu', 'pundi', 'ndi'],

            // Sanksi & keringanan
            'late_fee' => ['denda', 'terlambat', 'telat', 'keterlambatan', 'tunggak', 'menunggak', 'tunggakan', 'dendo', 'kelewatan pajak'],
            'fine_waiver' => ['pemutihan', 'penghapusan denda', 'hapus denda', 'relaksasi', 'keringanan', 'hapus dendo'],

            // Dokumen hilang & pemblokiran
            'lost_doc' => ['hilang', 'kehilangan', 'surat kehilangan', 'stnk hilang', 'bpkb hilang', 'ilang', 'kelangan', 'surat ilang'],
            'stnk_block' => ['blokir stnk', 'pemblokiran', 'blokir', 'blokir kendaraan', 'kendaraan dijual', 'didol', 'blokir pajak'],

            // Perpanjangan & pengesahan
            'annual_renewal' => ['tahunan', 'perpanjangan', 'perpanjang stnk', 'pajak tahunan', 'nganyari stnk'],
            'five_year_renewal' => ['5 tahunan', 'lima tahunan', 'ganti plat 5 tahunan', 'perpanjangan 5 tahunan', 'plat lima tahunan'],
            'proof_of_payment' => ['tbpkp', 'e-tbpkp', 'pengesahan', 'bukti bayar', 'struk', 'pengesahan elektronik'],

            // Perubahan data & komponen
            'name_transfer' => ['balik nama', 'bbn', 'bea balik nama', 'oper nama', 'balik jeneng'],
            'address_change' => ['ubah alamat', 'ganti alamat', 'pindah alamat', 'owah alamat'],
            'plate_change' => ['ganti plat', 'ubah plat', 'plat baru', 'genti plat', 'plat anyar'],
            'color_change' => ['ganti warna', 'ubah warna', 'genti werna'],
            'engine_change' => ['ganti mesin', 'ubah mesin', 'genti mesin'],
            'vehicle_type_change' => ['rubah bentuk', 'ubah bentuk', 'perubahan bentuk', 'ngowahi bentuk'],

            // Pajak & biaya
            'progressive_tax' => ['pajak progresif', 'progresif', 'kk sama', 'alamat sama', 'nomor kk sama'],
            'bbn_rates' => ['tarif bbn', 'bea balik nama', 'bbn berapa'],
            'swdkllj_info' => ['swdkllj', 'iuran wajib', 'jasaraharja'],

            // Layanan kanal Samsat
            'samsat_keliling' => ['samsat keliling', 'samling', 'keliling'],
            'samsat_drive_thru' => ['drive thru', 'drive through', 'drive-thru'],
            'samsat_corner' => ['samsat corner', 'gerai samsat', 'gerai'],
            'payment_channels' => ['indomaret', 'alfamart', 'pos', 'agen', 'collector'],

            // Proses teknis
            'cek_fisik' => ['cek fisik', 'gesek nomor rangka', 'gesek nomor mesin', 'gesek rangka'],
            'bank_va_payment' => ['virtual account', 'va', 'kode bayar', 'kode pembayaran'],
            'esamsat_issue' => ['gagal bayar', 'tidak bisa', 'error aplikasi', 'kendala aplikasi', 'server sibuk'],

            // Lain-lain relevan
            'vehicle_sold' => ['dijual', 'jual kendaraan', 'kendaraan dijual', 'didol'],
            'vehicle_lost' => ['kendaraan hilang', 'motor hilang', 'mobil hilang', 'ilang kendaraan'],
            'bpkb_pickup' => ['ambil bpkb', 'pengambilan bpkb'],
        ];
    ...
    /**
     * Compute how many of the user's intents are reflected in KB fields/tags.
     */
    private function kbIntentMatchScore(array $kb, array $userIntents): int
    {
        if (empty($userIntents)) return 0;
        $tagsStr = '';
        if (isset($kb['tags'])) {
            if (is_array($kb['tags'])) $tagsStr = implode(' ', array_map('strval', $kb['tags']));
            else $tagsStr = (string) $kb['tags'];
        }
        $hay = mb_strtolower((string) (($kb['title'] ?? '') . ' ' . ($kb['search_content'] ?? '') . ' ' . ($kb['answer'] ?? '') . ' ' . $tagsStr));
        $map = [
            'check_tax' => ['cek', 'check', 'cek pajak', 'cek biaya', 'cek tarif', 'tarif', 'biaya', 'lihat pajak', 'esamsat', 'piro pajek', 'pinten pajek', 'ndelok pajek', 'ncek pajek'],
            'online_payment' => ['bayar online', 'pembayaran online', 'e-samsat', 'mbanking', 'm-banking', 'atm', 'virtual account', 'va', 'bank jatim', 'bri', 'bni', 'mandiri', 'dana', 'ovo', 'shopee', 'mbayar online'],
            'offline_location_hours' => ['lokasi', 'alamat', 'jam operasional', 'jam buka', 'jam bukak', 'tutup', 'buka sabtu', 'pundi', 'ndi'],
            'late_fee' => ['denda', 'terlambat', 'telat', 'keterlambatan', 'tunggak', 'menunggak', 'tunggakan', 'dendo', 'kelewatan pajak'],
            'fine_waiver' => ['pemutihan', 'penghapusan denda', 'hapus denda', 'relaksasi', 'keringanan', 'hapus dendo'],
        ...
        $score = 0;
        foreach ($userIntents as $intent) {
            $keys = $map[$intent] ?? [];
            foreach ($keys as $k) {
                if ($k !== '' && mb_strpos($hay, $k) !== false) {
                    $score++;
                    break;
                }
            }
