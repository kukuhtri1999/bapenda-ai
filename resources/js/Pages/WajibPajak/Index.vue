  ...
  <div class="p-6">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <input v-model="q" @input="debouncedSearch" placeholder="Cari nama / nopol / WA" class="px-3 py-2 border rounded" />
        <button
          @click="search"
...
const data = ref({ data: [], current_page: 1, last_page: 1, from: 0, to: 0, total: 0 });

const fetchList = async (page = 1) => {
  loading.value = true;
  try {
    const res = await axios.get('/admin/wajib-pajak', { params: { page, q: q.value || undefined }, headers: { 'X-Requested-With': 'XMLHttpRequest' } });
...
const reset = () => { q.value = ''; fetchList(1); };
const goto = (page) => { if (!page || page < 1 || page > (data.value.last_page||1)) return; fetchList(page); };
