...
# Replace uploadFiles function to handle batch uploads
$oldUploadFunction = @'
...
$content = $content -replace [regex]::Escape($oldUploadFunction), $newUploadFunction

$content | Set-Content 'resources\js\Pages\PhotoEditing\Index.vue'
Write-Host "Updated uploadFiles function to handle batch uploads!"'@

$content | Set-Content 'fix_batch_upload.ps1'
Write-Host "Created batch upload fix script!"
